...
### US-015: GameController 深度集成 [体验优化]
> 关联功能: F-021
> 来源: INS-023 ~ INS-026 (📄 技术审计)

**作为** 使用 DualSense 手柄的用户
**我希望** 应用充分利用手柄的高级功能
**以便** 获得接近原生 PlayStation 的操控体验

**验收标准**:
- **AC-061**: DualSense 自适应扳机支持：解码 PS5 扳机效果事件并应用到手柄
- **AC-062**: 触控板位置追踪：支持需要触控板滑动操作的 PS5 游戏
- **AC-063**: Haptics 引擎统一：ControllerManager 与 HapticsManager 共享引擎实例
- **AC-064**: 控制器电池电量显示：在 UI 中展示连接手柄的电量状态

---

### US-016: 手柄操控 UI/UX 优化 [体验优化]
> 关联功能: F-022
> 来源: INS-028 ~ INS-031 (🎨 UI/UX 审查)

**作为** 完全使用手柄操控的用户
**我希望** 应用的 UI 布局针对手柄操作进行优化
**以便** 我可以快速访问常用功能而无需多层菜单导航

**验收标准**:
- **AC-065**: 主机快速操作栏：主机卡片支持快捷手势（如左滑/右滑）触发唤醒、设置、删除等操作，替代长按上下文菜单
- **AC-066**: 流媒体中音量快捷调节：支持 PS+L2/R2 组合键在流媒体中直接调节音量，无需打开菜单
- **AC-067**: PIN 输入数字键盘优化：PIN 输入界面提供手柄友好的数字键盘网格（3×4布局），支持方向键导航选择数字
- **AC-068**: 设置快捷入口：流媒体控制菜单中提供"快捷设置"入口，可直接访问分辨率、码率等常用设置而无需断开连接

---

### US-017: iPad 触摸操作友好化 [体验优化]
> 关联功能: F-023
> 来源: INS-032 ~ INS-039 (🎨 UI/UX 审查)

**作为** iPad 用户（触摸操作）
**我希望** UI 控件尺寸和间距符合 Apple HIG 触摸标准
**以便** 我可以准确点击目标而不容易误触

**验收标准**:
- **AC-069**: 触摸目标尺寸：所有可交互控件最小尺寸 ≥44×44pt（符合 Apple HIG）
- **AC-070**: 控件间距优化：相邻可交互控件间距 ≥16pt（防止误触）
- **AC-071**: Slider 交互区域：滑块可触摸高度扩大到 44pt
- **AC-072**: 触觉反馈一致性：所有可交互控件提供统一的触觉反馈（使用 HapticsManager）
